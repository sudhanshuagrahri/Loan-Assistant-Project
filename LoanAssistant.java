...
public class LoanAssistant extends JFrame {
	...
	JLabel intrestLabel = new JLabel();
	...
	public LoanAssistant()
	{
		...
		gridConstraints.anchor = GridBagConstraints.WEST;
		gridConstraints.insets = new Insets(10,10,0,0);
		getContentPane().add(intrestLabel,gridConstraints);
		...
		gridConstraints.gridy = 1;
		gridConstraints.insets = new Insets(10,10,0,10);
		getContentPane().add(intrestTextField,gridConstraints);
		intrestTextField.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent ae)
			{
				intrestTextFieldActionPerformed(ae);
			}
		});
	...
	private void computeButtonActionPerformed(ActionEvent ae) {
		...
		int months;
		double monthlyIntrest, multiplier;
		...
		{
			JOptionPane.showConfirmDialog(null, "Invalid or Empty Loan Balance entry.\nPlease Correct it!","Balance Input Error",
					JOptionPane.DEFAULT_OPTION,JOptionPane.INFORMATION_MESSAGE);
			return;
		}
		
		if(validateDecimalNumber(intrestTextField))
			intrest = Double.valueOf(intrestTextField.getText()).doubleValue();
		else
		{
			JOptionPane.showConfirmDialog(null, "Invalid or Empty Intrest Rate Entry.\nPlease Correct it!","Intrest Input Error",
					JOptionPane.DEFAULT_OPTION,JOptionPane.INFORMATION_MESSAGE);
			return;
		}
		...
		else
		{
			...
			if(validateDecimalNumber(paymentTextField))
			{
				...
				if(payment<=(balance*monthlyIntrest+1.0))
				{
					...
					{
						paymentTextField.setText(new DecimalFormat("0.00").format((int)(balance*monthlyIntrest+1.0)));
						payment = Double.valueOf(paymentTextField.getText()).doubleValue();
					}
					else
					{
		...
		//reset payment prior to analysis to fix at two decimal place
		payment = Double.valueOf(paymentTextField.getText()).doubleValue();
		//show analysis
		analysisTextArea.setText("Loan Balance: $"+new DecimalFormat("0.00").format(balance));
		analysisTextArea.append("\n"+"Intrest Rate: "+new DecimalFormat("0.00").format(intrest)+"%");

		// proccess all but last payment
		loanBalance = balance;
	...
	private void intrestTextFieldActionPerformed(ActionEvent ae) {
	...
	{
		//checks that textfield contains valid decimal number
		//with only digits and and single decimal point
		boolean valid = true;
		...
		else
		{
			for(int i=0; i<s.length(); i++)
			{
				...
				if(c>='0' && c<='9')
					...
					valid = false;		// invalid charcter found
			}
		}
